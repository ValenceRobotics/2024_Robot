...
public class ChaseTagCmd extends Command {
    ...
    private static final ProfiledPIDController xController = new ProfiledPIDController(0, 0, 0, X_CONSTRAINTS);
    private static final ProfiledPIDController yController = new ProfiledPIDController(0, 0, 0, Y_CONSTRAINTS);
    private static final ProfiledPIDController oController = new ProfiledPIDController(0, 0, 0, O_CONSTRAINTS);

    private static final Transform3d goalPose = new Transform3d(new Translation3d(1, 0, 0), new Rotation3d(0, 0, Math.PI));

    public ChaseTagCmd(DriveSubsystem dt, AprilTagCamera cam) {
        this.dt = dt;
        this.cam = cam;
  ...
  // Called every time the scheduler runs while the command is scheduled.
    ...
    public void execute() {
        ...
        if (result.hasTargets()) {
            ...
            dt.drive(xSpeed, ySpeed, oSpeed, true, true);
  ...
  public boolean isFinished() {
    return xController.atGoal() && yController.atGoal() && oController.atGoal();
  }
