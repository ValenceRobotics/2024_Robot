...
public class PivotSubsystem extends ProfiledPIDSubsystem {
  ...
  private static final double kMaxVel = 2*Math.PI;
  private static final double kMaxAccel = 0;

  private static final double kG = 0.68;
  private static final double kV = 0.97;
  ...
  public PivotSubsystem(){
    ...
    setGoal(PivotConstants.kHomePosition);
  ...
  public void useOutput(double output, TrapezoidProfile.State setpoint) {
    ...
    double feedforward_value = feedforward.calculate(setpoint.position, setpoint.velocity);
    // Add the feedforward to the PID output to get the motor output
    setPivotVoltage(output+feedforward_value);
  }
